...
import 'package:flutter_account/views/accounting/search/SearchIndex.dart';
import 'package:flutter_account/views/accounting/setting/setting_index.dart';
...
class BillItemIndex extends StatefulWidget {
  const BillItemIndex({Key? key}) : super(key: key);

  @override
  State<BillItemIndex> createState() => BillItemIndexState();
}

class BillItemIndexState extends State<BillItemIndex>
  ...
  Map<String,String> categoryMap = {"吃饭":"Expenses:Eat", "交通":"Expenses:Traffic", "购物":"Expenses:Shopping", "住房":"Expenses:House", "社交":"Expenses:Relationship"};
  ...
  Future<void> loadBillItemsByMonth() async {
    ...
    // 请求线上账本列表
    CusDataResult temp =
        await _dbHelper.queryBillItemListApi(selectedMonth, "");
    var newData = temp.data as List<BillItem>;
    allBillItems = newData;
          ...
          onChanged: (String? newValue) {
            if (newValue != null) {
              setState(() {
                ...
                // 筛选数据，更新数据
                var value = categoryMap[selectedCategory];
                print("选择的分类:$value");
                setState(() {
                  // 恢复billItems数据
                  billItems = allBillItems;
                  // 筛选billItems的account等于value的账单
                  if(value!= null){
                    billItems = billItems.where((element) =>
                    element.account!.substring(0, element.account!.lastIndexOf(':')) == value).toList();
                  }
                  print("筛选后的数据:${billItems}");
                  billItemGroupByDayMap = groupBy(billItems, (item) => item.date);
                });
              });
            }
          },
          items: ["全部",...categoryMap.keys].map<DropdownMenuItem<String>>((String value) {
...
// 显示日期选择器
            showMonthPicker(
              context: context,
              headerColor: Color.fromARGB(255, 255, 220, 68),
          ...
          GestureDetector(
            ...
            onLongPress: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => SettingIndex(), // 替换为你的设置页面
                ),
              );
            },
            child: Image.asset('images/search.png', width: 44, height: 44),
          ),
        ],
      ),
